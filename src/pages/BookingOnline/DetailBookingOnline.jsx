...
function DetailBookingOnline() {
  ...
  const detailBooking = () => {
    Gym.detailBookingOnline(id).then((res) => setDetail(res.data.data));
  };
  console.log(detail);
  const handleSubmit = (e) => {
    e.preventDefault();
    PostApi.setStatusBookingOnline({ id, status })
      ...
      <div className="flex m-3 p-3 gap-x-5  border   ">
        ...
        <div className="w-[40%]  flex-col justify-center ">
          <div className=" flex gap-x-14 font-semibold text-md">
            ...
            <div className="capitalize">
              <p>: {detail.online_class?.title}</p>

              <p>
                : <FormatRupiah value={detail.online_class?.price} />{" "}
              </p>
...
export default DetailBookingOnline;
